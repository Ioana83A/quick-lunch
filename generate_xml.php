...
$root_element = "meniu_curent"; //fruits
$xml         .= "<meniuri_QL>\n";
      ...
      foreach($result_array as $key => $value)
      {
		 ...
		 if(strcmp($key,"ziua")==0)
		 {
		 	if(strcmp($value,"luni")==0)
			  $data = $days_this_week[0]->format('Y-m-d');
			else if(strcmp($value,"marti")==0)
			  $data = $days_this_week[1]->format('Y-m-d');
			else if(strcmp($value,"miercuri")==0)
			  $data = $days_this_week[2]->format('Y-m-d');
			else if(strcmp($value,"joi")==0)
			  $data = $days_this_week[3]->format('Y-m-d');
			else if(strcmp($value,"vineri")==0)
			  $data = $days_this_week[4]->format('Y-m-d');
		 }
      }
       $xml .= "<data>\n";
	   $xml .= "<![CDATA[$data]]>\n";
